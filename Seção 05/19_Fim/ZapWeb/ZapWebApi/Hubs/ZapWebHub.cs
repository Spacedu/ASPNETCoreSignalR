...
namespace ZapWebApi.Hubs
{
    public class ZapWebHub : Hub
    {
        ...
        public async Task CriarOuAbrirGrupo(string usuarioEmailOne, string usuarioEmailTwo)
        {
            ...
            var mensagens = Banco.Mensagens.Where(a => a.GrupoNome == grupo.Nome).OrderBy(a=>a.Criado).ToList();

            await Clients.Caller.SendAsync("AbrirGrupo", grupo.Nome, mensagens);
        }
        ...
        {
            var usuarios = Banco.Usuarios.OrderBy(a=>a.Nome).ToList();
            await Clients.Caller.SendAsync("ReceberUsuarios", usuarios);
        }
    }
}
